...
void WhisperStream::startWhisper()
{
    mDone = false;
    mThr = std::thread(std::bind(&WhisperStream::work, this));
}

void WhisperStream::stopWhisper()
{
    mDone = true;
    if(mThr.joinable()){
        mThr.join();
...
void WhisperStream::printf(const char *fmt, ...)
{
    va_list argptr;
    va_start(argptr, fmt);
...
void WhisperStream::work()
{
    ...
    if(!ctx){
        return printf("error. whisper context is null");
    }
    ...
    while (is_running) {
        // handle Ctrl + C
        is_running = !mDone;//sdl_poll_events();
